...
class Ball {
  ...
  void display() {
    ...
    noStroke();
    //colorMode(RGB);
    clr = (int)map(lineNum, 0, 20, 140, 240);
  ...
  void saveHistory(float x, float y, float z) {
  ...
  void lineBetween(ArrayList others) {
    ...
    for (int i = 0; i < others.size (); i ++) {
      ...
      if (distance >0 && distance < disCheck) { //dis range: 0 ~ 75
        ...
        /*
        //pshapestyle
         PShape line = createShape(LINE, pos.x, pos.y, pos.z, otherBall.pos.x, otherBall.pos.y, otherBall.pos.z);
         line.setStroke(color(hue, sat, bri));       
         group.addChild(line);
         */
      }
    }
